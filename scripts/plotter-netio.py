...
def plot(csvFilePaths, options, tags):
    ...
    for t in tags:
        ...
        if t.tagType == TAG_TYPE.XY:
            ...
            if "per_feat" in t.name:
                try:
                    ax2
                except NameError:
                    ax2 = plt.gca().twinx()
                ax2.plot(dedup_exes, dedup_whys,
                ...
                plt.ylabel("GB per Iteration per Feature")
            else:
                plt.plot(dedup_exes, dedup_whys,
                    label=(options.custom_legend_labels[tagIndex] if options.custom_legend_labels != None else t.name),
                    color=(dracula.COLORS[tagIndex+options.color_offset] if options.color_theme == "dracula" else None))
                plt.ylabel("GB per Iteration")
    ...
    if len(legendArtistProxyShapes) > 1 and not skipLegend: # 3dbar plot = special legend
        ...
        if options.horizontal:
            ...
            xlims = plt.xlim()
            ylims = plt.ylim()
            ylims = ylims[0], ylims[1]+0.049 # make extra room for legend

            # side by side bars on y axis
            # plt.vlines(xlims[0], ylims[0], ylims[1], color=dracula.COLORS[0], lw=1, zorder=4, clip_on=False)
            # plt.vlines(xlims[0]+1.4, ylims[0], ylims[1], color=dracula.COLORS[1], lw=1, zorder=4, clip_on=False)
            # plt.vlines(xlims[1], ylims[0], ylims[1], color=dracula.COLORS[2], lw=1, zorder=4, clip_on=False)
            # plt.vlines(xlims[1]-1.4, ylims[0], ylims[1], color=dracula.COLORS[3], lw=1, zorder=4, clip_on=False)

            # split vertical
            midpt = (ylims[1] - ylims[0])/1.4 # magic
            plt.vlines(xlims[0], ylims[0], midpt, color=dracula.COLORS[1], lw=1, zorder=4, clip_on=False)
            plt.vlines(xlims[0], midpt, ylims[1], color=dracula.COLORS[0], lw=1, zorder=4, clip_on=False)
            plt.vlines(xlims[1], ylims[0], midpt, color=dracula.COLORS[3], lw=1, zorder=4, clip_on=False)
            plt.vlines(xlims[1], midpt, ylims[1], color=dracula.COLORS[2], lw=1, zorder=4, clip_on=False)
            ...
            #    if side == 0:
            #        spine.set_edgecolor(dracula.COLORS[0], dracula.COLORS[1])
            #    if side == 1:
            #        spine.set_edgecolor(dracula.COLORS[2])
            #    side += 1
