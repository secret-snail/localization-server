// Adapted from
//  https://github.com/mez/monocular-visual-odometry/blob/master/main.cpp
//  https://github.com/avisingh599/mono-vo/blob/master/src/visodo.cpp
//      explained here https://avisingh599.github.io/vision/monocular-vo/
// One has MIT license, one does not... Not sure who copied whom.
#include <ctype.h>
#include <algorithm>
#include <ctime>
#include <fstream>
 ...
 ***************** Algorithm Outline
    ...
    3. Use FAST algorithm to detect features in It, and track those features to
 It+1. A new detection is triggered if the number of features drop below a
 certain threshold.
    4. Use Nister’s 5-point alogirthm with RANSAC to compute the essential
 matrix.
    5. Estimate R,t from the essential matrix that was computed in the previous
 step.
    6. Take scale information from some external source (like a speedometer),
...
// const static char* dataset_images_location =
// "/home/mez/dataset/kitti/dataset/sequences/00/image_1"; const static char*
// dataset_poses_location =
// "/home/mez/dataset/kitti/odomerty_poses/poses/00.txt";
const static char* dataset_images_location =
    "../../data-kitti/dataset/sequences/00/image_1";
const static char* dataset_poses_location =
    "../../data-kitti/dataset/poses/poses/00.txt";
...
vector<double> getAbsoluteScales() {
  ...
  if (myfile.is_open()) {
    while (getline(myfile, line)) {
      ...
      scales.push_back(sqrt((x - x_prev) * (x - x_prev) +
                            (y - y_prev) * (y - y_prev) +
                            (z - z_prev) * (z - z_prev)));
    }
    myfile.close();
...
int main(int argc, char** argv) {
  ...
  /*
   * Projection matrix example after rectification: Right Grey scale Camera
   * (3x4)
   *
  ...
  Mat E, R, t, mask;
  E = findEssentialMat(points2, points1, focal, pp, cv::RANSAC, 0.999, 1.0,
                       mask);
  ...
  cv::namedWindow("Road facing camera | Top Down Trajectory",
  ...
  for (int numFrame = 2; numFrame < MAX_FRAME; numFrame++) {
    ...
    E = findEssentialMat(currFeatures, prevFeatures, focal, pp, cv::RANSAC,
                         0.999, 1.0, mask);
    ...
    // This is cheating because ideally you'd want to figure out a way to get
    // scale, but without this cheat there is a lot of drift.
    ...
    circle(
        traj,
        cv::Point(groundPoses[numFrame].x + 600, groundPoses[numFrame].y + 100),
        1, CV_RGB(0, 255, 0), 2);

    rectangle(traj, cv::Point(10, 30), cv::Point(550, 50), CV_RGB(0, 0, 0),
              cv::FILLED);
    ...
    if (renderFeatures) {
      ...
      for (auto point : currFeatures)
        cv::drawMarker(currImage_c, cv::Point(point.x, point.y),
                       CV_RGB(0, 255, 0), cv::MARKER_TILTED_CROSS, 2, 1,
                       cv::LINE_AA);
    }
